...
from FLAlgorithms.trainmodel.latent_mapping import latent_mapping
...
class User:
    ...
    def __init__(
        ...
        self.latent_mapping = latent_mapping(inputDim=args.KD_latent_size).cuda()
        # self.latent_to_logit = latent_to_logit(inputDim=args.KD_latent_size,)
        self.optimizer_latent_mapping = pFedIBOptimizer(self.latent_mapping.parameters(), lr=self.learning_rate)
        self.lr_scheduler_latent_mapping = torch.optim.lr_scheduler.ExponentialLR(optimizer=self.optimizer_latent_mapping, gamma=0.99)
    ...
    def get_next_train_batch(self, count_labels=False):
        ...
        result = {'X': X, 'y': y}
        if count_labels:
            unique_y, counts=torch.unique(y, return_counts=False)
