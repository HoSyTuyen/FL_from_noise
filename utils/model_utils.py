...
def create_model(model, dataset, algorithm):
    passed_dataset = get_dataset_name(dataset)
    model= Net(passed_dataset, algorithm), model
...
def get_gen(generator, latent_size, batch_size):
...
def get_logit(image, model, z, train=True):
    if train:
        model.train()
        logit = model(image)
    else:
    ...
    # model.train()
    return logit['output_z'], z

def get_gen_with_optim_latent(generator, model, latent_size, batch_size):
    z = torch.randn(batch_size, latent_size, 1, 1).cuda()
    z_op = model(z)
    z_op = z_op.reshape(batch_size, latent_size, 1, 1)
